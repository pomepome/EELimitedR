...
public class ItemDMShears extends ItemEETool
{
	public ItemDMShears()
	{
		super(UnlocalizedRegistry.DMShearsID, 8, 6);
	}
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
    {
    	...
    	int radius = getChargeLevel(stack) + 2;
    	...
    	for(int dx = -radius;dx <= radius;dx++)
    	{
    		for(int dy = -radius;dy <= radius;dy++)
    		{
    			for(int dz = -radius;dz <= radius;dz++)
    			{
    				...
    				Block block = world.getBlock(mx, my, mz);
    				int metadata = world.getBlockMetadata(mx, my, mz);
    				if(isLeaves(block))
    				{

    					List<ItemStack> sheared = ((IShearable)block).onSheared(stack, world, mx, my, mz, 0);

    					for(ItemStack is : sheared)
    					{
    						WorldUtils.spawnEntityItem(world, is, player);
    					}
