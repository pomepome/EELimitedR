...
public class EntityLavaProjectile extends EntityThrowable
{
	...
	public void onUpdate()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			if (this.posY > 127 && worldObj.isRaining())
			{
				if(InventoryUtils.useResource((EntityPlayer)thrower,18,true))
				{
	...
	protected void onImpact(MovingObjectPosition mop)
	{
		...
		if (mop.typeOfHit == MovingObjectType.BLOCK)
		{
			...
			if(!InventoryUtils.useResource((EntityPlayer)thrower, 128,true))
			{
				return;
			}
		...
		else if (mop.typeOfHit == MovingObjectType.ENTITY)
		{
			...
			if(!InventoryUtils.useResource((EntityPlayer)thrower, 8,true))
			{
				ent.setFire(1);
			}
			else
